...
1. An API key from Pure (contact us at [collectpure.com](https://www.collectpure.com) to request one)
...
```bash
curl -X GET "https://public.api.collectpure.com/protected" \
  -H "x-api-key: YOUR_API_KEY"
```

## Making Your First Request

Let's verify your API key is working by making a request to the authentication test endpoint:
...
```bash
curl -X GET "https://public.api.collectpure.com/spot-prices" \
  -H "x-api-key: YOUR_API_KEY"
```
...
```bash
curl -X GET "https://public.api.collectpure.com/product/PRODUCT_ID" \
  -H "x-api-key: YOUR_API_KEY"
```
...
```bash
curl -X GET "https://public.api.collectpure.com/products?offset=0&limit=10" \
  -H "x-api-key: YOUR_API_KEY"
```
...
async function getSpotPrices() {
  try {
    const response = await axios.get(`${BASE_URL}/spot-prices`, {
      headers: {
        "x-api-key": API_KEY,
      },
    });
...
def get_spot_prices():
    ...
    response = requests.get(f'{BASE_URL}/spot-prices', headers=headers)
...
3. Contact us at [collectpure.com](https://www.collectpure.com) if you have any questions or need assistance
